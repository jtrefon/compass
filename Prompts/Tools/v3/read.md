...
**When to use:** Reading files. Use `ls` first to find the file path — `read` only works on files, not directories.
...
- Path is a directory: Use `ls` to list directory contents — `read` only opens files.
