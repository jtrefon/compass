...
**Expected output:** Plain text. A header `Search results for: "<query>" (N results)` followed by one numbered entry per result, each showing a title, `URL:`, and a snippet. It ends with a hint to use `web_fetch` (action=open) to read a specific page.
Example:
```
...
[2] Configuring Strict in TS
    ...
    ...
Use web_fetch with action=open and a url from above to read a specific page.
```
